...
# --- 1. QUẢN LÝ DỮ LIỆU (FIXED LOADING) ---
@st.cache_data
def load_all_data():
...
# Hàm hỗ trợ tìm dữ liệu kể cả khi tên Level bị lệch nhẹ
def get_content(data_dict, target_level):
    ...
    # Thử tìm khớp tương đối (Ví dụ: "Level_A1" khớp với "Level_A1 (Cơ bản)")
...
with st.sidebar:
    ...
    # Gom tất cả các Level từ cả 4 file để người dùng luôn có lựa chọn
    all_keys = []
...
if mode == "Từ vựng":
    ...
    if not v_list: st.warning(f"Không tìm thấy từ vựng cho '{level}' trong vocab.json")
    else:
...
# 4.2 TRẮC NGHIỆM
    ...
    if not q_list: st.warning(f"Chưa có câu hỏi trắc nghiệm cho '{level}' trong quiz.json")
    else:
        ...
        with st.form("quiz_form"):
            ...
            if st.form_submit_button("Xác nhận câu trả lời"):
                if choice == q['answer']:
                    st.success("Chính xác! Đang chuyển câu..."); reset_task(); st.rerun()
                else: st.error("Chưa đúng rồi!")

# 4.3 READING
elif mode == "Reading":
    r_list = get_content(bundle['read'], level)
    if not r_list: st.warning(f"Trống bài đọc cho '{level}' trong reading.json")
    else:
...
# 4.4 WRITING
    ...
    if not w_list: st.warning(f"Trống dữ liệu cho '{level}' trong writing.json")
    else:
