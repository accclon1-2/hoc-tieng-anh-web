...
# --- CONFIG ---
st.set_page_config(page_title="UTH English Pro v6.2", layout="wide")
...
# 5.1 TỪ VỰNG
if mode == "Từ vựng":
    v_data = get_content(bundle['vocab'], level); v_list = v_data.get("vocabulary", []) if v_data else []
        ...
        with st.form("vocab_form"):
            ...
            if st.form_submit_button("Kiểm tra"):
                if ans.strip().lower() == correct.strip().lower():
                    ...
                    st.session_state.score_feedback = "Chính xác! Bạn thật tuyệt vời. 🎉"
                    # Logic SRS
                    ...
                    st.session_state.score_feedback = f"Chưa đúng rồi. Đáp án là: **{correct}**"
    ...
    if not q_list: st.warning("Trống trắc nghiệm.")
        ...
        with st.form("quiz_form"):
            ...
            if st.form_submit_button("Xác nhận"):
                if choice == q['answer']:
                    st.session_state.is_correct = True
                    st.session_state.score_feedback = "Quá chuẩn! Bạn đã chọn đúng đáp án."
                else:
                    st.session_state.is_correct = False
                    st.session_state.score_feedback = f"Sai rồi Kiệt ơi. Đáp án đúng là: **{q['answer']}**"
        ...
        with col2:
            with st.form("reading_form"):
                ...
                if st.form_submit_button("Nộp bài"):
                    correct_count = sum(1 for i, qs in enumerate(r['questions']) if u_ans[i] == qs['a'])
                    st.session_state.is_correct = (correct_count == len(r['questions']))
                    st.session_state.score_feedback = f"Kết quả: {correct_count}/{len(r['questions'])} câu đúng."

            if st.session_state.score_feedback:
                if st.session_state.is_correct: st.success(f"Tuyệt vời! {st.session_state.score_feedback}")
                else: st.warning(f"Cố gắng lên! {st.session_state.score_feedback}")
                if st.button("Bài học mới"): reset_task(); st.rerun()
        ...
        with st.form("write_form"):
            ...
            if st.form_submit_button("Kiểm tra"):
                if user_w.strip().lower().replace(".", "") == t['answer'].strip().lower().replace(".", ""):
                    st.session_state.is_correct = True
                    st.session_state.score_feedback = "Viết rất tốt! Câu văn hoàn toàn chính xác."
                else:
                    st.session_state.is_correct = False
                    st.session_state.score_feedback = f"Gợi ý đáp án: **{t['answer']}**"

        if st.session_state.score_feedback:
            if st.session_state.is_correct: st.success(st.session_state.score_feedback)
            else: st.info(st.session_state.score_feedback)
            if st.button("Câu tiếp theo ⏭️"): reset_task(); st.rerun()
